# Meerza Ahmed  
# USC CSCI 570 Final Project - Memory-Efficient 
...
def efficient_alignment_func(seqX, seqY):  
     
    if not seqX or not seqY:
        return (
            seqX or "_" * len(seqY),
            seqY or "_" * len(seqX),
            (len(seqX) + len(seqY)) * delta
        )
        
    if len(seqX) == 1 or len(seqY) == 1:
     return Alignment_func(seqX, seqY)

    # Divide and conquer
    splitX = len(seqX) // 2
    
    forward = compute_dp_row(seqX[:splitX], seqY)      
    backwards = compute_dp_row(seqX[splitX:][::-1], seqY[::-1])  
    
    splitY = min(
        range(len(seqY) + 1), 
        key=lambda j: forward[j] + backwards[len(seqY) - j]
    )
    
    # Recursive calls
    newX1, newY1, cost1 = efficient_alignment_func(seqX[:splitX], seqY[:splitY])
    newX2, newY2, cost2 = efficient_alignment_func(seqX[splitX:], seqY[splitY:])
    return newX1 + newX2, newY1 + newY2, cost1 + cost2

def compute_dp_row(subX, subY):
    
    x, y = len(subX), len(subY)
    row = [i * delta for i in range(y + 1)]
    newRow = [0] * (y + 1)
    ...
    for i in range(1, x + 1):
        ...
        for j in range(1, y + 1):
            c2 = subY[j - 1]
            cm = row[j - 1] + alpha[c1][c2]
            cx = row[j] + delta
            cy = newRow[j - 1] + delta
            newRow[j] = min(cm, cx, cy)
...
def time_wrapper(seqX,seqY):
    ...
    newX, newY, cost = efficient_alignment_func(seqX, seqY)  
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        print("Usage: python3 efficient_3.py inputFile.txt outputFile.txt")
        sys.exit(1)
